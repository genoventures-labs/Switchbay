...
const BUILTIN_GUIDES: Guide[] = [
  ...
  {
    id: "memory-helper-quick-start",
    title: "Memory Helper Quick Start",
    description: "How models should use the Memory Helper engine to search and recall from Switchbay memory stores on demand.",
    kind: "quickstart",
    triggers: ["memory", "remember", "recall", "notes", "facts", "knowledge", "what did i", "what was", "search memory", "memory helper"],
    source: "builtin",
    path: "builtin/memory-helper-quick-start",
    body: [
      "Memory and knowledge are NOT pre-loaded into context. Use the Memory Helper engine to fetch them when you need them.",
      "",
      "## Tools",
      "- `search_memory` — search across all stores and get ranked hits with source, path, score, and snippet.",
      "- `recall_memory` — fetch the best-matching content as a compact context pack ready to use.",
      "",
      "## Sources you can target (pass via `sources` param)",
      "context, switchbay, notes, facts, summary, knowledge, pins, sessions, plan, guides — or 'all'",
      "",
      "## When to call it",
      "- The user asks what was remembered, noted, or decided about something.",
      "- You need workspace facts, project notes, or prior decisions before answering.",
      "- You are about to give advice that might contradict a standing preference or boundary.",
      "",
      "## Examples",
      "```",
      "search_memory(query: \"API keys\", sources: \"context,notes\", scope: \"global\")",
      "recall_memory(query: \"active projects\", scope: \"workspace\")",
      "recall_memory(query: \"working style preferences\", sources: \"context\", scope: \"global\")",
      "```",
      "",
      "## Rules",
      "- Prefer `recall_memory` when you need usable content; use `search_memory` when you want to discover where something is stored.",
      "- Do not claim something is not remembered until you have searched for it.",
      "- Scope to `workspace` when the question is project-specific; scope to `global` for personal preferences and cross-project facts.",
    ].join("\n"),
  },
  {
    id: "spawn-agent-quick-start",
    title: "Spawn Agent Quick Start",
    description: "How models should use spawn_agent to delegate focused subtasks to subagents.",
    kind: "quickstart",
    triggers: ["spawn agent", "subagent", "delegate", "parallel", "spawn_agent", "multi-agent"],
    source: "builtin",
    path: "builtin/spawn-agent-quick-start",
    body: [
      "## When to use spawn_agent",
      "- The task has a self-contained subtask that can be fully described in a single prompt and whose result you will synthesize back into your own answer.",
      "- The subtask benefits from a clean context (no parent transcript noise) or a specialist agent persona.",
      "- Common patterns: delegate a code audit to `reviewer`, research to a generalist, and a DB schema review to `architect` — then synthesize all three results yourself.",
      "",
      "## When NOT to use it",
      "- The subtask requires interactive back-and-forth or depends on your own in-progress reasoning.",
      "- You are already inside a subagent (spawn_agent is not available to subagents).",
      "- The task is trivial — direct tool calls are faster than a full nested loop.",
      "",
      "## Usage",
      "```",
      "spawn_agent(",
      "  prompt: \"Review src/agent/loop.ts for correctness and edge cases. Return a bullet list of findings.\",",
      "  agent_id: \"reviewer\",   // optional: activate a specialist persona",
      "  context: \"<file contents here>\",  // optional: pass data you already have",
      "  label: \"code review\"   // shown in step display",
      ")",
      "```",
      "",
      "## Rules",
      "- Write self-contained prompts — the subagent has no memory of your conversation.",
      "- Pass relevant file contents via `context` rather than asking the subagent to re-read them if you already have them.",
      "- Subagents run up to 12 tool iterations; scope the prompt accordingly.",
      "- After the subagent returns, synthesize its output into your own response — do not just echo it verbatim.",
    ].join("\n"),
  },
  {
