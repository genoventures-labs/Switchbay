...
const STEPS: Step[] = [
  ...
  { key: "servers", question: "Which LM Studio MCP servers should it use?", hint: "Names like playwright, filesystem, github, memory.", required: true },
  { key: "integrations", question: "Exact integration IDs if you know them?", hint: "Optional. Example: mcp/playwright, mcp/filesystem", required: false },
    ...
    return (
      <Box flexDirection="column" paddingX={2} paddingY={1} marginBottom={1} borderStyle="round" borderColor={brandColor} backgroundColor={TUI_COLORS.baseDeep}>
        <Box gap={2}>
          ...
          <Text color={grayColor}>Using the Cloud lane to draft the JSON...</Text>
        </Box>
      </Box>
    );
  }
