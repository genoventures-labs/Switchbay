...
export type CreatePluginAnswers = {
  name: string;
  purpose: string;
  contents: string;
  notes: string;
...
const STEPS: Step[] = [
  ...
  { key: "contents", question: "What real parts should it eventually bundle?", hint: "Optional. agents, skills, engines, knowledge, mcp.", required: false },
...
type CreatePluginDrawerProps = {
  generating: boolean;
  onCancel: () => void;
  onComplete: (answers: CreatePluginAnswers) => void;
  visible: boolean;
};

export function CreatePluginDrawer({ generating, onCancel, onComplete, visible }: CreatePluginDrawerProps) {
  const [stepIndex, setStepIndex] = useState(0);
  const [current, setCurrent] = useState("");
  const [answers, setAnswers] = useState<CreatePluginAnswers>({
    name: "",
    purpose: "",
    contents: "",
    notes: "",
  ...
  if (generating) {
    return (
      <Box flexDirection="column" paddingX={2} paddingY={1} marginBottom={1} borderStyle="round" borderColor={brandColor} backgroundColor={TUI_COLORS.baseDeep}>
        <Box gap={2}>
          <Text color={brandColor} bold>Creating plugin</Text>
          <Text color={grayColor}>Writing a bounded Switchbay plugin manifest...</Text>
        </Box>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" paddingX={2} paddingY={1} marginBottom={1} borderStyle="round" borderColor={brandColor} backgroundColor={TUI_COLORS.baseDeep}>
      <Box gap={2} marginBottom={1}>
        <Text color={brandColor} bold>Create Plugin</Text>
